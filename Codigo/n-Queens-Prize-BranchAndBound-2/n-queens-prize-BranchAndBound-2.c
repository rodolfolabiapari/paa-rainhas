 ...
 */
int Le_Premios(char * diretorio, int ** premios, float ** media_coluna){
	FILE * file;
...
float Calcula_Fator_Continua_Recursao(int tabuleiro_atual_premio, int tabuleiro_maior_premio, float * media_coluna, int coluna_atual, int filtro) {
	...
	if (global_imprime_tela_ativado || global_imprime_filtro) {
		printf("\n[INFO %8d] Premio Atual: %d;\tPeso c/ Media: %f;\tMaior Premio: "
				"%d\tFator Continua Recursão: %.7f Filtro: %d", cont++, tabuleiro_atual_premio, 
				tabuleiro_atual_premio + media_coluna[coluna_atual], tabuleiro_maior_premio, fator, filtro);
		fflush(stdout);
	}
	
	// return (float) tabuleiro_atual->premio / tabuleiro_maior->premio;
 ...
 */
Tabuleiro * n_Rainhas_Prize(int coluna_atual, Tabuleiro * tabuleiro_atual, 
...
int main(int argc, char** argv) {
	...
	maior_premio = Le_Premios(diretorio, &premios, &media_coluna_premios);
